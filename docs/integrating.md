# Integrating DrawdownCap

## From Solidity

```solidity
import {DrawdownCapHook} from "drawdown-cap/src/hooks/DrawdownCapHook.sol";
...
hook.configure(key, DrawdownCapHook.Config({ /* see the README */ }));
...
const address = hookAddress("drawdown-cap", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "DrawdownCap"
...
More at https://drawdown-cap.pages.dev.
