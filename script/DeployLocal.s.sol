...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(DrawdownCapHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new DrawdownCapHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, DrawdownCapHook.Config({maxFallTicks: 500, epochSeconds: 3600}));
